(() => {
      ...
      if (bodyLockStatus) {
        ...
        if (this.targetOpen.element) {
          ...
          const positionButtons =
            popupContent.querySelectorAll('.position-button');
  ...
  function initShowMoreBtns() {
    if (document.getElementsByClassName('comments__slider')) {
      ...
      for (let i = 0; i < slides.length; i++)
        if (
          slides[i]
            .querySelector('.slide-comm__text')
            .classList.contains('needShowMore')
        )
          slides[i].querySelector('.slide-comm__button').style.display =
            'block';
        else
          slides[i].querySelector('.slide-comm__button').style.display = 'none';
    }
  }
    ...
    addEventListener('load', function () {
      ...
      function show() {
        document.body.style.paddingRight =
          ...
          'px';
        document.documentElement.classList.add('lock');
        const preloadTextPosHolder = document.querySelector(
          '#preload-text-position-holder'
        );
        let targetTop = preloadTextPosHolder.getBoundingClientRect().top + 'px';
        let targetLeft =
          preloadTextPosHolder.getBoundingClientRect().left + 'px';
        ...
        setTimeout(function () {
          let preloadTextStartPositionX =
            document.getElementsByTagName('body')[0].clientWidth / 2 -
            preloadTextPosHolder.getBoundingClientRect().width / 2 +
            'px';
          ...
          setTimeout(function () {
            preloadTextOriginal.animate(
              preloadTextChangePosition,
              preloadTextTiming
            );
            document
              .querySelector('.preload-bg')
              .animate(bgTransform, bgTransformTiming);
          }, durationTextMoving);
          setTimeout(function () {
          ...
          let animatedMediaBlockTexts = document.querySelectorAll(
            '.media-block__animate-text'
          );
          setTimeout(function () {
            if (animatedMediaBlockTexts)
          ...
          setTimeout(function () {
            let animatedMediaBlockImages =
              document.querySelectorAll('._img-anim-hidden');
            if (animatedMediaBlockImages)
              animatedMediaBlockImages.forEach((animatedBlockImage) => {
                animatedBlockImage.classList.add('animated');
              });
          }, durationTextAppear + durationTextMoving + waitMediaTextMoving);
          setTimeout(function () {
            let animatedMediaBlockTextsOrigin =
              document.querySelectorAll('._text-anim-origin');
            if (animatedMediaBlockTextsOrigin)
              animatedMediaBlockTextsOrigin.forEach(
                (animatedBlockTextOrigin) => {
            ...
            durationTextMoving +
            waitMediaTextMoving +
            durationMediaTextMoving);
        }, waitTextAppear);
      }
      function skip() {
        preloadTextOriginal.style.display = 'none';
        document.querySelector('#preload-text-position-holder').style.opacity =
          '100%';
        document.querySelector('.preload-bg').style.display = 'none';
        let animatedMediaBlockTexts = document.querySelectorAll(
          '.media-block__animate-text'
    ...
    if (document.getElementsByClassName(className))
      for (
        ...
        document
          ...
          [i].insertAdjacentHTML('afterbegin', htmlCode);
  }
  addEventListener('load', function () {
    new Rellax('.rellax');
  });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (numbersColumns)
      numbersColumns.forEach((numbersColumn) => {
        for (var i = 1; i <= 50; i++)
          numbersColumn.insertAdjacentHTML('beforeend', i + '<br>');
      });
  });
  ...
  function getCascadeSliderOffsetY() {
    cascadeSliderOffsetY =
      cascadeSlider.getBoundingClientRect().top + window.scrollY;
  }
  ...
  function animateCascadeSlide() {
    ...
    if (
      window.scrollY > cascadeSliderOffsetY &&
      window.scrollY < cascadeSliderOffsetYBottom
    )
      for (let i = 0; i < cascadeSlidesCount; i++) {
        let cascadeSlideScale =
          1 -
          (0.04 *
            (window.scrollY -
              cascadeSliderOffsetY +
              56 -
              cascadeSlideHeight * i)) /
            cascadeSlideHeight;
        let cascadeSlideOpacity =
          1 -
          (0.2 *
            (window.scrollY -
              cascadeSliderOffsetY +
